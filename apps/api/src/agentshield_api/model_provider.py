...
def _unavailable(message: str) -> ModelUnavailable:
    telemetry.increment("model_errors_total", type="unavailable")
...
class VerifiedArtifactModel:
    ...
    def __init__(self, *, path: str, expected_sha256: str, version: str, metadata_path: str | None = None) -> None:
        ...
        if not artifact.is_file():
            raise _unavailable("risk model artifact path does not exist")
        actual = hashlib.sha256(artifact.read_bytes()).hexdigest()
        if actual.lower() != expected_sha256.lower():
            raise _unavailable("risk model artifact checksum mismatch")

        if metadata_path:
            metadata_file = Path(metadata_path)
            if not metadata_file.is_file():
                raise _unavailable("risk model metadata path does not exist")
            try:
                metadata = json.loads(metadata_file.read_text(encoding="utf-8"))
            except (OSError, ValueError) as exc:
                raise _unavailable("risk model metadata could not be read") from exc
            metadata_sha = str(metadata.get("artifact_sha256", ""))
            if metadata_sha.lower() != actual.lower():
                raise _unavailable("risk model metadata checksum mismatch")
            if metadata.get("feature_version") != FEATURE_VERSION:
                raise _unavailable("risk model feature version mismatch")
            if metadata.get("model_version") != version:
                raise _unavailable("risk model version mismatch")
            if metadata.get("status") not in {"APPROVED", "ACTIVE"}:
                raise _unavailable("risk model is not APPROVED/ACTIVE")

        try:
            loaded: Any = joblib.load(artifact)
        except Exception as exc:  # noqa: BLE001 - convert deserialization failures to a serving error
            raise _unavailable("risk model artifact could not be loaded") from exc
        if not hasattr(loaded, "predict_proba"):
            raise _unavailable("risk model artifact does not expose predict_proba")
        self._model = loaded
    ...
    def predict(
        ...
        *,
        ...
        if missing:
            raise _unavailable(f"risk model input features missing: {','.join(missing)}")
        ...
        try:
            ...
            raise _unavailable("risk model prediction failed") from exc
    ...
    def get_active(self) -> RiskModel:
        ...
        if self._artifact_model is None:
            if not settings.risk_model_artifact_path or not settings.risk_model_artifact_sha256:
                raise _unavailable("No verified APPROVED/ACTIVE risk model artifact is configured")
