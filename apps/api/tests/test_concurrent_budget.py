...
from agentshield_api.models import Agent, AgentBudgetState, AgentPolicy, Merchant, PaymentOrder, RiskDecision, Transaction
...
async def test_concurrent_payment_orders_do_not_exceed_daily_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async with SessionLocal() as session:
        ...
        await session.execute(delete(PaymentOrder).where(PaymentOrder.transaction_id.in_(transaction_ids)))
...
@pytest.mark.integration
async def test_concurrent_payment_orders_same_transaction_create_one_provider_order(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(settings, "require_operator_auth", False)
    monkeypatch.setattr(settings, "app_env", "development")
    monkeypatch.setattr(settings, "razorpay_key_id", "rzp_test_same_transaction")
    monkeypatch.setattr(settings, "razorpay_key_secret", "test-secret")
    ...
    async with SessionLocal() as session:
        session.add(Agent(id=agent_id, name="Payment Order Concurrency Agent", status="ACTIVE"))
        session.add(Merchant(id=merchant_id, name="Payment Order Merchant", category="SOFTWARE"))
        session.add(
            AgentPolicy(
                id=uuid4(),
                agent_id=agent_id,
        ...
        session.add(
            Transaction(
                ...
                amount=Decimal("30.00"),
                currency="INR",
                device_id="same-transaction-device",
                occurred_at="2026-09-05T10:00:00+00:00",
                status="EVALUATED",
            )
        )
    ...
    async def place_order(idempotency_key: str) -> int:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://testserver") as client:
            response = await client.post(
                "/api/v1/payments/orders",
                json=PaymentOrderRequest(transaction_id=transaction_id, idempotency_key=idempotency_key).model_dump(mode="json"),
            )
            return response.status_code

    statuses = await asyncio.gather(place_order("same-transaction-a"), place_order("same-transaction-b"))
    assert sorted(statuses) == [200, 409]
    assert provider_calls == 1

    async with SessionLocal() as session:
        orders = (await session.scalars(select(PaymentOrder).where(PaymentOrder.transaction_id == transaction_id))).all()
        assert len(orders) == 1
        state = await session.scalar(
            select(AgentBudgetState).where(
                AgentBudgetState.agent_id == agent_id,
        ...
        assert state is not None
        assert state.spent == Decimal("30.00")
        assert state.reserved == Decimal("0.00")

        await session.execute(delete(PaymentOrder).where(PaymentOrder.transaction_id == transaction_id))
        await session.execute(delete(RiskDecision).where(RiskDecision.transaction_id == transaction_id))
