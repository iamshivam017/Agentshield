...
from agentshield_api.models import Agent, AgentPolicy, AuditEvent, Merchant, PolicyEvaluation, RiskDecision, RiskPrediction, Transaction, TransactionFeature
...
async def test_investigation_api_persists_provenance_and_replays(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(app.state, "_agentshield_test_marker", True, raising=False)
    ...
    async with SessionLocal() as session:
        ...
        session.add(AgentPolicy(id=policy_id, agent_id=agent_id, version=1, is_active=True, rules={"transaction_limit": "1000.00", "daily_limit": "5000.00", "verification_threshold": "0.25", "allowed_categories": ["SOFTWARE"]}))
        session.add(Transaction(id=transaction_id, agent_id=agent_id, merchant_id=merchant_id, amount=Decimal("250.00"), currency="INR", device_id="investigation-device", occurred_at="2026-09-05T10:00:00+00:00", status="EVALUATED"))
        await session.flush()
        ...
        session.add(PolicyEvaluation(id=uuid4(), transaction_id=transaction_id, policy_version=1, result="VERIFY", violations=[]))
        ...
        session.add(AuditEvent(id=uuid4(), transaction_id=transaction_id, event_type="RISK_DECISION_CREATED", actor_type="AGENT", actor_id=str(agent_id), payload={"decision": "VERIFY"}))
        await session.commit()

    headers = {"X-Operator-API-Key": "", "X-Operator-ID": "risk-analyst"}
    ...
    async with SessionLocal() as session:
        from agentshield_api.models import Investigation
        investigation = await session.get(Investigation, body["result"] and body.get("transaction_id") and UUID(str(transaction_id))) if False else None
        investigation = await session.scalar(__import__("sqlalchemy").select(Investigation).where(Investigation.transaction_id == transaction_id))
        ...
        await session.execute(delete(PolicyEvaluation).where(PolicyEvaluation.transaction_id == transaction_id))
